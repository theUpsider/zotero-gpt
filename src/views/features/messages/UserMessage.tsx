...
function SelectionToggle({
  ...
  return (
    <div className="inline" onClick={handleClick}>
      ...
      <span
        className={`ml-2 capitalize ${selected ? "text-tomato" : "text-neutral-500"}`}
      >
        {header} ({state.length})
      </span>
    </div>
...
function StatesInfo({ states }: StatesInfoProps) {
  ...
  return (
    <div className="flex flex-col text-sm mt-1">
      ...
      {selectedName ? (
        <SelectionDetail name={selectedName} state={states[selectedName]} />
      ) : null}
    </div>
  )
}
      ...
      {images.map(({ id, title, image }: States["images"][number]) => (
        ...
        >
          <div
            ...
            style={{ backgroundColor: selectionConfig.images.backgroundColor }}
          >
...
export const UserMessage = memo(function UserMessageContent({
  ...
  const isShortMessage =
    ref?.current?.offsetWidth && ref.current?.offsetWidth < 64
  ...
  function handleCopy() {
    ...
    new ztoolkit.Clipboard()
      .addText(content.newPlainTextValue, "text/unicode")
      .copy()
    setCopyId(id)
  }
